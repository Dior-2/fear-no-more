...
export default function FilterComponent() {
  ...
  const [limit] = useState(parseInt(listData.length / 4));
  let projectList = listData.slice(nextPage, nextPage + 4);

  useEffect(() => {
    async function fetchData() {
      const getAll = await axios.get(`http://18.222.198.9/api/listings/requests`)
        .then(({ data }) => {
          setListData(data);
        })
        .catch((error) => {
  ...
  function filterByCategory(option) {
    ...
    function fetchData() {
      const getFilteredData = axios.get(`http://18.222.198.9/api/listings/requests`, {
        params: {
